      ...
      <table class="table table-bordered table-hover">
        ...
        <tbody>
          <tr v-for="(customer, index) in customers.data" :key="index">
            <td>{{ customer.number  }} - {{customer.type}}</td>
...
export default {
  ...
  methods: {
    ...
    getNumberDetails() {
      ...
      axios.get('http://localhost:8000/api/numberdetails', {
        headers: {
    ...
    checkCustomerData(number) {
      const token = localStorage.getItem('auth_token');
      axios.get(`http://localhost:8000/api/UploadMasterData/${number.number}`, {
        headers: {
          Authorization: `Bearer ${token}` // Send user token for authentication
        }
      })
      .then(response => {
        ...
        //   // If number exists in the master data, navigate to the customer data page
          this.$router.push({ name: 'CustomerDataPage', params: { customerId: response.data.customerId } });
        // } else {
      ...
      .catch(error => {
        ...
        alert('Error checking customer data. Please try again.');
      });
    }
  }
};
